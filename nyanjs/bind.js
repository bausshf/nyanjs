      ...
      if (events) {
        ...
        for (var i = 0; i < eventEntries.length; i++) {
          var eventEntry = eventEntries[i].split(':'),
              eventHandler = eventEntry.length === 2 ? viewController[eventEntry[1]] : undefined;

          if (!eventHandler) {
            continue;
          }

          viewController.controller.events.detach(eventEntry[0]);
          element.removeEventListener(eventEntry[0], element.__detachEvent);

          viewController.controller.events.attach(eventEntry[0], function() {
              eventHandler.apply(this, [element, model].concat(Array.prototype.slice.call(arguments)));
          });

          element.__detachEvent =function() {
            viewController.controller.events.fire(eventEntry[0], Array.prototype.slice.call(arguments))
          };
